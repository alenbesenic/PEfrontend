<template>
     <v-container fluid>
    <v-row>
        <v-card max-width="400" v-for="event in events" :key="event.id" outlined>
          ...
          <v-card-actions>
            ...
            <v-dialog :key="event.id" hide-overlay max-width="600">
              ...
              <v-card max-width="600px" max-height="1000px" outlined>
...
export default {
  ...
  methods:{
    async getUpcoming(){
      this.events = await Events.getUpcomingEvents()
      console.log("Upcoming", this.events)
    },
    goToEventPage(a){
      window.open(a)
    }
